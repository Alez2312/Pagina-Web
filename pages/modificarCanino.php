        ...
        <form class="canino_form" method="POST" name="canino" id="canino" action="procesoModificarCanino.php" onsubmit="return validated()" enctype="multipart/form-data">
            ...
            while ($row = mysqli_fetch_assoc($resultado)) { ?>
                <div class="font">
                    ...
                    <input type="hidden" name="id" id="id" value="<?php echo $row['id_canino']; ?>"><?php echo $row['id_canino']; ?>
                </div>
                <div class="font font2">
                    <label class="nombre">Nombre:</label>
                    <input type="text" name="nombre" id="nombre" value="<?php echo $row['nombre_canino']; ?>" required>
                </div>
                <div class="font font3">
                    <label class="fecha_adopcion_inicial">Fecha adopción inicial:</label>
                    <input type="text" name="fecha_adopcion_inicial" id="fecha_adopcion_inicial" value="<?php echo $row['fecha_adopcion_inicial']; ?>" required>
                </div>
                <div class="font font4">
                    <label class="fecha_adopcion_final">Fecha adopción final:</label>
                    <input type="text" name="fecha_adopcion_final" id="fecha_adopcion_final" value="<?php echo $row['fecha_adopcion_final']; ?>" required>
                </div>
